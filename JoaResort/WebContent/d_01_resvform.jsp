  	...
  	<table width border=1 cellspacing=0 cellpadding=5 align =center style="border-right: hidden; border-left: hidden">
			<tr>
				...
				<script>/*여기서 부터는 특수문자 입력방지 코드이다.*/    
					    ...
					    $(document).ready(function(){//jQuery 사용
					      ...
					      //설명란 textarea 체크
							$('#comment').on("keyup keypress keydown",function(e){
							    var content = $(this).val();
							    $('#counter1').html("("+content.length+" / 최대 200자)");    //글자수 실시간 카운팅
						
							    if (content.length > 200){
							        alert("최대 200자까지 입력 가능합니다.");
							        $(this).val(content.substring(0, 200));//첨부터 마지막 200번째까지 글자수 자름
							        $('#counter1').html("(200 / 최대 200자)");//자른 200자를 기재
							    }
							});
							//설명란 textarea 체크
							$('#address').on("keyup keypress keydown",function(e) {
								...
								if (content.length > 70) {
									...
									$(this).val(content.substring(0, 70));//첨부터 마지막 70번째까지 글자수 자름
									$('#counter').html("(70 / 최대 70자)");//자른 70자를 기재
								}
							});
			...
			<tr>
				...
				<%
						...
						out.println("<select name=room class=\"form-control\">");
						for(int i=0;i<30;i++){
						%>
						<option value=<%=ld.plusDays(i+1)%>><%=ld.plusDays(i+1)%></option>
			...
			<tr>
				...
				<td bgcolor=white>
					...
					<%
						...
						{		
							case "1":
								out.println("<option value=1>일반룸</option>");
								break;
							case "2":
								out.println("<option value=2>VIP룸</option>");
								break;
							case "3":
								out.println("<option value=3>합리적인 룸</option>");
								break;
							case "0":
								out.println("<option value=1>일반룸</option>");
								out.println("<option value=2>VIP룸</option>");
			...
			<tr>
				...
				<td bgcolor=white><input type="text" name="address" id="address" class=".DOC_TEXT" size=70> <br />
				<span style="color: #aaa;" id="counter">(0 / 최대 70자)</span></td>
			<!-- 	<script>
					//설명란 textarea 체크
					$('#address').on("keyup keypress keydown",function(e) {
						console.log("key up Success");
			...
			<tr>
				...
				<td bgcolor=white><input type=text  class="form-control" name="phonenumber" size=40 maxlength=70/></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<textarea width=70 name="comment" id="comment" class=".DOC_TEXT2,form-control"></textarea>
				<br /><span style="color:#aaa;" id="counter1">(0 / 최대 200자)</span></td>
				<!-- <script>
					//설명란 textarea 체크
					$('#comment').on("keyup keypress keydown",function(e){
					    var content = $(this).val();
